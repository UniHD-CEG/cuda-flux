...
extern "C"
uint64_t* __attribute__((weak)) createBBCounterMemory(size_t len) {
...
extern "C"
uint32_t __attribute__((weak)) getProfilingMode() {
...
extern "C"
struct timeval* __attribute__((weak)) startClock() {
...
extern "C"
void __attribute__((weak)) serializeBBCounter(uint64_t *d_blockList, size_t len, uint32_t n_banks,
    const char* kernelname, uint64_t gridXY, uint32_t gridZ,
    uint64_t blockXY, uint32_t blockZ, uint64_t shmsize, cudaStream_t stream,
    struct timeval* t1) {
  ...
  output << string(kernelname) << ", "
    ...
    << gridZ << ", "
    << (uint32_t)(blockXY & 0x00000000FFFFFFFF) << ", "
...
extern "C"
void __attribute__((weak)) mekong_WritePTXBlockAnalysis(bool *isWritten, const char* analysis) {
