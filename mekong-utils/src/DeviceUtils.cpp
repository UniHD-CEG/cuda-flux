...
///===------------------------------------------------------------------===//
  ...
  llvm::Function* cloneAndAddArgs(llvm::Function *source, std::vector<llvm::Type*> argType, std::vector<std::string> name) {
    ...
    FunctionType *cloneType = FunctionType::get(source->getReturnType(), cloneArgs, false);

    Function *clone = Function::Create(cloneType, source->getLinkage(), source->getName() + "_clone", source->getParent());
  ...
  /// Adds metadata to the current module that will make the given function callable as kernel
  ...
  void markKernel(llvm::Module& m, llvm::Function* kernel) {
    ...
    if (kernelMD) {
      MDNode *node = MDNode::get(ctx, {ValueAsMetadata::get(kernel), MDString::get(ctx, "kernel"),
                                       ValueAsMetadata::get(ConstantInt::get(Type::getInt32Ty(ctx), 1, true))});
