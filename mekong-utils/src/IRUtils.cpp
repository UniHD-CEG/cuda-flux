...
void linkIR(llvm::StringRef ir, llvm::Module &m) {
  ...
  std::unique_ptr<llvm::Module> irModule =
      llvm::parseIR(llvm::MemoryBufferRef(*llvm::MemoryBuffer::getMemBuffer(ir, "", true)), err, ctx);
...
llvm::Function *getPrintfFunc(llvm::Module &module) {
  ...
  //llvm::FunctionType *printf_type = llvm::TypeBuilder<int(char *, ...), false>::get(module.getContext());
  std::vector<Type*> args = { Type::getInt8PtrTy(ctx)};
  FunctionType *printf_type = FunctionType::get(Type::getInt32Ty(ctx), args, true);
  Function *func = llvm::cast<Function>(module.getOrInsertFunction("printf", printf_type).getCallee());
...
llvm::CallInst *callPrintf(llvm::Module &module, std::string str, llvm::Value *val) {
  ...
  llvm::ArrayType *type = llvm::ArrayType::get(llvm::IntegerType::get(ctx, 8), str.size() + 1);
  llvm::Constant *strConstant = llvm::ConstantDataArray::getString(ctx, str);
  llvm::GlobalVariable
      *GVStr = new llvm::GlobalVariable(module, type, true, llvm::GlobalValue::InternalLinkage, strConstant);
