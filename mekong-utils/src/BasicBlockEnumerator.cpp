...
void expand(BasicBlock *node, std::set<BasicBlock *> &visited, std::deque<BasicBlock *> &frontier) {
  ...
  if(BranchInst *bi = dyn_cast_or_null<BranchInst>(ti)) {
    for(BasicBlock * successor : bi->successors()) {
      ...
      if (visited.find(successor) == visited.end()
          and not contains(successor, frontier)) {
...
namespace mekong {
  void visitNodes( BasicBlock *begin, BasicBlock *end, void *data, bool (*visitorFunc) (BasicBlock *, void *)) {
